class MyColor {
  constructor() {
    ...
    this.orange = [255, 165, 0];

    this.dark_red = [100, 0, 0];
    this.dark_red2 = [150, 0, 0];
    this.dark_blue = [0, 0, 100];
    this.dark_blue2 = [0, 0, 150];
    this.dark_green = [0, 100, 0];
    this.dark_yellow = [100, 100, 0];
    this.dark_black = [0, 0, 0];
    this.dark_gray = [100, 100, 100];
  }
}
...
class myPlot {
  ...
  rectangle_top_bottom(top, bottom, col) {
    const pos = [
      top,
      [top[0], bottom[1]],
      bottom,
  ...
  rectangle_tb_wired(top, bottom, col, width) {
    const pos = [
      top,
      [top[0], bottom[1]],
      bottom,
    ...
    this.line(pos[0], pos[1], col, width);
    this.line(pos[1], pos[2], col, width);
    this.line(pos[2], pos[3], col, width);
    this.line(pos[3], pos[0], col, width);
  }

  rectangle_pos_size(pos, size, col) {
    this.rectangle_top_bottom(pos, [pos[0] + size[0], pos[1] + size[1]], col);
  }

  point(pos, col, size = 10) {
    this.triangle(pos, [pos[0] + size, pos[1]], [pos[0], pos[1] + size], col);
  }

  line(p1, p2, col, size = 10) {
  ...
  wired_circle(pos, size, col, width) {
    const dth = 6.28 / 20.0;
    for (let i = 0; i < 20; i++) {
      const p1 = [
        pos[0] + Math.cos(dth * i) * size,
